...
export class IcccService {
  ...
  async getVehicleDetectionLogs(imei: string, from: string, to: string) {
    ...
    const logs = await this.prisma.vehicleDetectionLog.findMany({
      where: {
        imei: deviceImei,
  ...
  // ────────────────────────────────────────────────────────────────
  // Everything below is disabled per request — kept for reference,
  // not deleted. Re-enable by uncommenting if garbage/ANPR/audio
  // endpoints are needed again.
  // ────────────────────────────────────────────────────────────────
